...
//   bun run sandcastle:plan-implement-review
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const plan = await sandcastle.run({
    ...
    promptFile: "./.sandcastle/plan-implement-review/plan-prompt.md",
    ...
    issues.map(async (issue) => {
      ...
      try {
        ...
        const implement = await sandbox.run({
          ...
          maxIterations: 100,
          agent: sandcastle.claudeCode("claude-opus-4-8"),
          promptFile: "./.sandcastle/plan-implement-review/implement-prompt.md",
          promptArgs: {
            TASK_ID: issue.id,
        ...
        if (implement.commits.length > 0) {
          const review = await sandbox.run({
            ...
            maxIterations: 1,
            agent: sandcastle.claudeCode("claude-opus-4-8"),
            promptFile: "./.sandcastle/plan-implement-review/review-prompt.md",
            promptArgs: {
              BRANCH: issue.branch,
            },
          });
  ...
  await sandcastle.run({
    ...
    maxIterations: 1,
    agent: sandcastle.claudeCode("claude-opus-4-8"),
    promptFile: "./.sandcastle/plan-implement-review/merge-prompt.md",
    promptArgs: {
